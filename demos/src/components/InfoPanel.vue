<template>
  <div class="info-panel">
    ...
    <div class="info-panel-text" v-if="currentView === 'mnist-cnn'">
      Interactive demo of a simple convnet trained on MNIST. All computation performed entirely in your browser. The 2D layers and images are also shown below the 3D visualization.
    </div>
    <div class="info-panel-text" v-else-if="currentView === 'mnist-vae'">
      Modified from the MNIST convolution/deconvolution variational autoencoder example <a target="_blank" href="https://github.com/fchollet/keras/blob/master/examples/variational_autoencoder_deconv.py">here</a>. The network demonstrated here is the generative decoder portion. The network generates an image through a series of Conv2DTranspose layers from coordinates in the 2D latent space. All computation performed entirely in your browser. Toggling GPU on/off shouldn't reveal any significant speed differences, as this is a fairly small network. The 2D layers and images are also shown below the 3D visualization.
    </div>
    <div class="info-panel-text" v-else-if="currentView === 'mnist-acgan'">
      Interactive demo of MNIST digit generation, using the generator network trained in an auxillary classifier generative adversarial network (AC-GAN). Modified based on original GitHub repo <a target="_blank" href="https://github.com/lukedeo/keras-acgan">lukedeo/keras-acgan</a>. During training the discriminator must both tease apart real images from fake synthetic images, as well as perform classification on the images. Here, during generation, we can condition upon the digit class. The 2D layers are also shown below the 3D visualization.
    </div>
    ...
    <div class="info-panel-text" v-else-if="currentView === 'squeezenet-v1.1'">
      In contrast to ResNet-50 and Inception-V3, the size of the weights for SqueezeNet is relatively minimal, less than 5 MB. We use the architecture and pretrained weights from <a target="_blank" href="https://github.com/rcmalli/keras-squeezenet">rcmalli/keras-squeezenet</a> (original at <a target="_blank" href="https://github.com/DeepScale/SqueezeNet">DeepScale/SqueezeNet</a>). Enter any valid image URL as input to the network. You can also select from a list of prepopulated image URLs. The endpoint must have CORS enabled, to enable us to extract the numeric data from the canvas element, so not all URLs will work. Imgur and <a target="_blank" href="https://www.flickr.com/search/?text=&license=2%2C3%2C4%2C5%2C6%2C9&sort=interestingness-desc">Flickr creative commons</a> all work, and are good places to start. After running the network, the top-5 classes are displayed. Keep in mind also we are limited to the <a target="_blank" href="https://github.com/transcranial/keras-js/blob/master/demos/src/utils/imagenet.js">1,000 classes of ImageNet</a>. Keep in mind that this is image classification and not object detection, so the network is forced to output a single class through softmax. Best results are on images where the classification target spans a large portion of the image. All computation performed entirely in your browser. The 2D layers are also shown below the 3D visualization.
    </div>
